...
TPI <- read.csv("S:/COS/PyroGeog/amartinez/Persistance/Intermediates/Variables/Iterators/TPI/TPI_Sample.csv", header = T, na.strings = -9999)
TRI <- read.csv("S:/COS/PyroGeog/amartinez/Persistance/Intermediates/Variables/Iterators/TRI/TRI_Sample.csv", header = T, na.strings = -9999)
TRASP <- read.csv("S:/COS/PyroGeog/amartinez/Persistance/Intermediates/Variables/Iterators/TRASP/TRASP_Sample.csv", header = T, na.strings = -9999)
SWASP <- read.csv("S:/COS/PyroGeog/amartinez/Persistance/Intermediates/Variables/Iterators/SWASP/SWASP_Sample.csv", header = T, na.strings = -9999)
valuetable <- cbind(vegsamp, TPI$TPI, TRI$TRI, TRASP$TRASP, SWASP$SWASP)
colnames(valuetable) <- c(colnames(vegsamp), "TPI", "TRI", "TRASP", "SWASP")

LookUp <- read.csv("S:/COS/PyroGeog/amartinez/Persistance/Intermediates/LandCover/GapAttrib.csv")
valuetable$UI[valuetable$UI == 2] <- "Burned"
valuetable$UI[valuetable$UI == 5] <- "Persistent Unburned"
...
valuetable$ForRan <- LookUp$CL[match(unlist(valuetable$LCvr.Gap), LookUp$Value)]
valuetable$ForRan <- Reclass$new[match(unlist(valuetable$ForRan), Reclass$old)]

valuetable <- na.omit(valuetable)
valuetable <- valuetable[valuetable$ForRan %in% c("Forest", "Range"),]

##
med2 <- ddply(valuetable, c("UI", "ForRan"), summarise, # calculate medians
             TPI.med = median(TPI), TRI.med = median(TRI), TRASP.med = median(TRASP), SWASP.med = median(SWASP))

ggplot(valuetable, aes(x=TRI, colour=UI)) + 
  geom_density(size = 1) +
  geom_hline(yintercept=0, colour="white", size=1) +
  geom_vline(data = med2, aes(xintercept = TRI.med, colour=UI, linetype = UI), size = .5, show.legend = F) +
...
ggplot(valuetable, aes(x=TPI, colour=UI)) + 
  geom_density(size = 1) +
  geom_hline(yintercept=0, colour="white", size=1) +
  geom_vline(data = med2, aes(xintercept = TPI.med, colour=UI, linetype = UI), size = .5, show.legend = F) +
  annotate("text", y = 0.3, x = -20, label = paste("\n", sprintf('\u2190'), "Valley"), size = 10/2.5) +
  annotate("text", y = 0.3, x = 0, label = "Flat", size = 10/2.5) +
  annotate("text", y = 0.3, x = 20, label = paste("\nRidge", sprintf('\u2192')), size = 10/2.5) +
  facet_wrap(~ForRan)+

...
# 90 meter TPI, TRI, and Roughness
width <- 7 # 90 meter
med <- median(1:width^2)
f <- matrix(1, nrow = width, ncol = width)
TPI2 <- focal(dem, w=f, fun=function(x, ...) x[med] - mean(x[-med]), pad=TRUE, padValue=NA)
writeRaster(TPI2, "Intermediates/Variables/TPI_90", "GTiff", overwrite = T)
TRI2 <- focal(dem, w=f, fun=function(x, ...) sum(abs(x[-med]-x[med]))/(width^2-1), pad=TRUE, padValue=NA)
writeRaster(TRI2, "Intermediates/Variables/TRI_90", "GTiff", overwrite = T)
rough2 <- focal(dem, w=f, fun=function(x, ...) max(x) - min(x), pad=TRUE, padValue=NA, na.rm=TRUE)
writeRaster(rough2, "Intermediates/Variables/rough_90", "GTiff", overwrite = T)
